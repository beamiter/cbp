...
const std::vector<std::pair<sFreeCircle, float> >& circle_array)
{
    ...
    {
        //The status does not lie in the circle
...
sehs::PrimitiveActions(sKinodynamic currentvs, float step_length)
{
    ...
    else
    {
        ...
        for (int i = -speed_num; i <= speed_num; ++i)
        {
            ...
            max_steering_velocity = (max_steering_angle - aim_angle) / controlCycle;
...
bool sehs::HeuristicSearch(sKinodynamic start, sKinodynamic goal, std::vector<sKinodynamic>& trajectory)
{
    ...
    while(!frontier.elements.empty())
    {
        ...
        aim_angle = 0;
        if(heuristic_index < sizeof(circle_path) - 1) 
        {
            aim_angle = atan2(goal.y , goal.x);
            double k = 1.2;
            float next2_angle = atan(circle_path.at(heuristic_index + 1).first.center.y / circle_path.at(heuristic_index + 1).first.center.x);
            float next1_angle = atan(circle_path.at(heuristic_index).first.center.y / circle_path.at(heuristic_index).first.center.x);
            float next2_dist = pow(circle_path.at(heuristic_index + 1).first.center.y , 2) + pow(circle_path.at(heuristic_index + 1).first.center.x , 2);
            float next1_dist = pow(circle_path.at(heuristic_index).first.center.y , 2) + pow(circle_path.at(heuristic_index).first.center.x , 2);
            
            if(next1_dist > k * circle_path.at(heuristic_index).first.radius)
            {
                aim_angle = next1_angle;
            }
            else
            {
                aim_angle = next1_angle + (next2_angle - next1_angle) * (next1_dist / k * circle_path.at(heuristic_index).first.radius);
            }
