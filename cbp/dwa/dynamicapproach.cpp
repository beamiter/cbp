...
dynamicapproach::dynamicapproach(ros::NodeHandle nh)
: stepsize(0.15)
, steplength(0.6)
, controlCycle(0.9)
, yawtolerance(9.0)
...
dynamicapproach::dynamicapproach()
{
}

dynamicapproach::~dynamicapproach() {}

int dynamicapproach::collisiondetection(std::vector<geometry_msgs::Point32> ego, sorted_tree &obspts, sVehicleElem vehiclemodel)
{
    ...
    for (auto tp : ego)
    {
        ...
        functions::carmodel(tp, littlepoly, 1.2, 0.4, 0.4);
...
int dynamicapproach::collisiondetection(ivpathplanner::path ego, sorted_tree &obspts, sVehicleElem vehiclemodel)
{
...
int dynamicapproach::collisiondetection(std::vector<sVehicleStatus> ego, sorted_tree &obspts,
...
dynamicapproach::GenerateTrajectory(sVehicleStatus currentvs, sKinematic sk)
{
    trajectorys.clear();
...
float dynamicapproach::gValUpdate(sVehicleStatus cur, sVehicleStatus next)
{
    float gval = 0.0;
...
float dynamicapproach::hValUpdate(sVehicleStatus cur, sVehicleStatus goal)
{
...
sVehicleStatus dynamicapproach::dynamicWindowApproach(sVehicleStatus start, sVehicleStatus goal, unordered_t<sVehicleStatus> &came_from,
...
void dynamicapproach::obstaclemodify(ivpredict::ivmsgpredict objs, sorted_tree &nearestobs,
                                sorted_tree &rawobs, sorted_tree &dilatedobs, float carspeed)
{
    sorted_tree().swap(nearestobs);
    sorted_tree().swap(rawobs);
    sorted_tree().swap(dilatedobs);
    ...
    tp.z = 0.0;
    geometry_msgs::Polygon poly;
    functions::carmodel(tp, poly, 0.1, 1.8, 1.2);
        ...
        for (auto b : a.cell)
        {
            ...
            else
            {
                ...
                if (dis >= 15.0)
                {
                    ...
                    if (!rawobs[opt.x].count(opt))
                    {
                        rawobs[opt.x].insert(opt);
                    }
                }
                ...
                for (int c = -3; c <= 3; c ++)
                {
                    temp.x = tt.x + 0.1 * c;
                    temp.y = tt.y + 0.3;
                    if (!dilatedobs[temp.x].count(temp))
                    {
                        dilatedobs[temp.x].insert(temp);
                    }
                    temp.x = tt.x + 0.1 * c;
                    temp.y = tt.y - 0.3;
                    if (!dilatedobs[temp.x].count(temp))
...
* @param
*/
int dynamicapproach::reachgoal(sVehicleStatus currentvs, sVehicleStatus goalvs)
{
    float dis, theta;
...
dynamicapproach::reconstructPath(sVehicleStatus start, sVehicleStatus goal,
...
ivpathplanner::path dynamicapproach::pathinterface(ivmap::ivmapmsglocpos ivlocpos,
                    ...
                    sorted_tree sortedobjs)
{
    ...
    ROS_ERROR_STREAM("dynamicapproach failed!!!");
    return ivpath;
}

void dynamicapproach::testModule()
{
